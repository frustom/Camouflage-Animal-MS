...
%      pages     = {-}, 
%
% In this trial, a network is trained on camo and tested on clear.
%% Building the Network

% Creating/Labeling DS for training & testing
ClearImgs = imageDatastore('New Clear','IncludeSubFolders',true,'LabelSource','foldernames');
CamoImgs = imageDatastore('New Camo','IncludeSubFolders',true,'LabelSource','foldernames');

% Split clear images to test on smaller portion
[testImgs1,CleartestImgs] = splitEachLabel(ClearImgs,0.7,'randomized');

% Image Preprocessing
trainds = augmentedImageDatastore([227 227],CamoImgs,'ColorPreprocessing','gray2rgb');
testds = augmentedImageDatastore([227 227],CleartestImgs,'ColorPreprocessing','gray2rgb');
...
'every-epoch','MaxEpochs',50,'Plots','training-progress');
